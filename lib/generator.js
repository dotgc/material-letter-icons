...
exports.generateIcons = function(input, cb) {
    ...
    async.eachSeries(letters, function(letter, cbAsync) {
        ...
        generateLetterIcon(letter, dims, function(err) {
            // Handle errors
            if (err) {
                return cbAsync(err);
            }
...
function generateLetterIcon(letter, dims, cb) {
    ...
    var fileDimsDirectoryName = dims.width + 'x' + dims.height + '/';
    var outputSVGPath = rootDir + config.dist.path + config.dist.svg.outputPath + fileName + '.svg';

    var outputPNGDirectory = rootDir + config.dist.path + config.dist.png.outputPath + fileDimsDirectoryName;

    if (!fs.existsSync(outputPNGDirectory)) {
        fs.mkdirSync(outputPNGDirectory);
    }
    var outputPNGPath = outputPNGDirectory + fileName + '.png';
    ...
    svg2png(Buffer.from(letterSVG), dims)
